...
> **💡 アプリから変更できます**：本体の「⚙️ AI Settings」ボタンで、プロバイダー・モデル・
> 生成サイズ・hires・エフェクト透過化方式などを **JSONを開かずに** 変更できます
> (ComfyUI 起動中ならモデル一覧から選べます)。以下はファイルを直接編集する場合の説明です。
...
| `effect_transparency.method` | エフェクトの透過化方式 | `black_to_alpha`=黒→透過（発光系向き・標準） / `rembg`=AI切り抜き |

> **暗い色のエフェクトが消えてしまうとき（rembg 切り抜き）**：標準の `black_to_alpha` は
> 「黒い部分を透明にする」方式のため、dark系など暗い色のエフェクトは一緒に消えがちです。
> `effect_transparency.method` を `rembg` にすると、背景除去AI（背景除去ツールと同じ birefnet）で
> エフェクトの形を切り抜くので、暗い色も残せます。使うには `pip install rembg` が必要です
> （未導入のままでもエラーにはならず、自動で標準方式に戻ります）。
