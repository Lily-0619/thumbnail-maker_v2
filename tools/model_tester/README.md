...
ComfyUI を使って画像を生成・比較する独立ツール。**2つのモード**を持ちます。
本体（`ui/` `core/` `config/`）は一切変更しません。`tools/` への追加のみ。

- **かんたんモード（既定）** … プロンプト＋モデル＋アップスケール＋枚数を選んで押すだけ。
  画像生成の仕組みがわからなくても使えます。同じ条件で複数枚出して見比べられます。
- **⚙️ 詳細（全機能）モード** … AUTOMATIC1111 の「X/Y/Z plot」と同じ考え方で、
  **X 軸=列 / Y 軸=行 / Z 軸=グリッドを複数枚**に Checkpoint・LoRA・Steps・CFG・Sampler・
  Seed・Prompt S/R などを割り当てて一括生成し、**ラベル付き比較グリッド**にまとめます。
  LoRAスタック・Hires/アップスケール・clip skip・VAE上書き・img2img に対応（勉強用）。

画面左上のトグルで切り替えます。プロンプト・モデルは共通なので、切り替えても入力は消えません。
モデルは `ai_config.json` の `comfyui.ckpt_name` を既定で採用します。
...
1. **接続 / 一覧更新** … ComfyUI に接続してモデル一覧を取得（自動でも実行されます）。
2. **プロンプト** … 出したい絵を書く。**ネガティブ**に避けたいものを書く。
3. **モデル** … 使う checkpoint を選ぶ（既定は `ai_config.json` の設定）。
4. **アップスケール** … ONで高画質化（少し遅くなる）。OFFで速く。
5. **枚数** … 同じ条件で何枚出すか（見比べ用）。
6. **▶ 画像を生成** … 生成。結果はクリックで拡大。`outputs/model_tester/<日時>/` に保存。

細かい設定（steps/cfg/解像度/サンプラー等）は `ai_config.json` の既定値が使われます。
いじりたくなったら『⚙️ 詳細』へ。
