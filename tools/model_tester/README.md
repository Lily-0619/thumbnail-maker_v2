...
どちらのモードでも **「🔧 ワークフローを見る（勉強用）」** ボタンを押すと、いまの設定で
ComfyUI に送られる**ノード配線図**が別窓で開きます（左→右に処理が流れる）。
LoRA や Hires(アップスケール) を ON にするとノードが増えるので、「何が起きているか」を見て学べます。
ComfyUI 接続や生成は不要で、押せばすぐ図が出ます。

- **💾 JSONを保存** … いまのワークフローを API 形式 JSON で書き出します（`outputs/model_tester/`）。
  新しめの ComfyUI なら、この `.json` を画面にドラッグ＆ドロップすると配線が復元され、
  本物のノードエディタで触れます（復元できない古い版では、このアプリの図で学べます）。
- **📄 JSONを表示** … 同じ内容をその場でテキスト表示します。

> 追加インストールは不要です（図の描画は標準の tkinter を使用）。
