...
1. 左の **「処理する画像」エリアは `outputs/bg_removal` フォルダを参照**する（base_image_sorter の
   仕分けグリッドを流用）。起動時にそこにある画像を一覧表示。画像/フォルダを **D&D**（または
   「＋ 画像を追加」）するとフォルダへコピーされて一覧に出る。エクスプローラ等で直接置いたら **🔄 再読込**。
...
```
outputs/bg_removal/
├── （取り込んだ画像の一時保存）   ← D&D/追加した画像をここへコピー
├── PNG/        ← 💾保存した背景除去PNG（{元名}_{モデル}_{日時}_nobg.png）
└── 処理済み/    ← 保存したら元画像（一時保存分）をここへ移動
```

- **取り込み時**：元ファイルは **コピー** で一時保存（外部の元画像は触らない＝非破壊）。
- **💾保存時**：背景除去PNGを `PNG/` へ、一時保存した元画像を `処理済み/` へ移動して一覧から外す。
- 同じ画像で複数モデルを保存しても、元画像の移動は1回だけ（PNGはモデルぶん増える）。
- 一覧は `outputs/bg_removal` 直下を参照。**🔄 再読込** でフォルダの変更（手動で置いた/消した）を反映。
