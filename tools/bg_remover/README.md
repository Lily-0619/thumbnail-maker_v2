...
7. 仕上げに **✏️ 手動編集**（透過消しゴム・Canva風）でブラシ補正できる：
   - **削除する**＝塗ったところを透過 / **復元する**＝塗ったところを元画像から戻す
   - サイズ調整・「元の画像を表示」ガイド・「↺ 編集をリセット」あり
   - **✔ 適用** でその結果に反映 → あとは 💾 で保存
8. 結果プレビュー全体は **🧹 プレビューをリセット** で実行前（元画像のみ）に戻せる。
...
| `mask_editor.py` | 手動切り抜き（透過消しゴム）エディタ `MaskEditorWindow` |
