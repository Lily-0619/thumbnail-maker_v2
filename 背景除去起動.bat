@echo off
chcp 65001 > nul
rem Background remover launcher. Runs from the bat's own folder,
...
if exist ".\.venv\Scripts\python.exe" (
    ".\.venv\Scripts\python.exe" tools\bg_remover\bg_remover_app.py
) else (
...
if errorlevel 1 (
    ...
    if exist ".\.venv\Scripts\python.exe" (
        echo   ".\.venv\Scripts\python.exe" -m pip install -r tools\bg_remover\requirements.txt
    ) else (
        echo   python -m pip install -r tools\bg_remover\requirements.txt
    )
)
pause
